...
describe Dessert do
  let(:chef) { double("chef", name:"Kudo") }
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do 
      ingredients = ["flour","eggs","chocolate","milk"]
  ...
  describe "#eat" do
    ...
    it "raises an error if the amount is greater than the quantity" do
      expect {cookie.eat(999) }.to raise_error("not enough left!")
    end
  end

  describe "#serve" do
    it "contains the titleized version of the chef's name" do
       allow(chef).to receive(:titleize).and_return("Chef Kudo")
       expect(cookie.serve).to eq("Chef Kudo has made 12 cookies!")
    end
  end
